...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
    ...
    public RadialPickerLayout(Context context, AttributeSet attrs) {
        ...
        mGrayBox.setLayoutParams(new ViewGroup.LayoutParams(
                ...
                ViewGroup.LayoutParams.MATCH_PARENT));
        mGrayBox.setBackgroundColor(getResources().getColor(
     ...
     * @param is24HourMode
     */
    public void initialize(Context context, int initialHoursOfDay,
                           int initialMinutes, boolean is24HourMode, boolean vibrate) {
        if (mTimeInitialized) {
            Log.e(TAG, "Time has already been initialized.");
            return;
        }
        mIs24HourMode = is24HourMode;
        mHideAmPm = Utils.isTouchExplorationEnabled(mAccessibilityManager) ? true
                : mIs24HourMode;
        ...
        if (!mHideAmPm) {
            mAmPmCirclesView.initialize(context, initialHoursOfDay < 12 ? AM
                    : PM);
        ...
        mHourRadialTextsView.initialize(res, hoursTexts,
                (is24HourMode ? innerHoursTexts : null), mHideAmPm, true);
        mHourRadialTextsView.invalidate();
        mMinuteRadialTextsView.initialize(res, minutesTexts, null, mHideAmPm,
                false);
        ...
        int hourDegrees = (initialHoursOfDay % 12)
                * HOUR_VALUE_TO_DEGREES_STEP_SIZE;
        mHourRadialSelectorView.initialize(context, mHideAmPm, is24HourMode,
                true, hourDegrees, isHourInnerCircle(initialHoursOfDay));
    ...
    private void setItem(int index, int value) {
        if (index == HOUR_INDEX) {
            ...
            mHourRadialSelectorView.setSelection(hourDegrees,
     ...
     * @param forceHigherOrLower The output may be forced to either the higher or lower step,
     ...
     * @param isInnerCircle       Whether the selection should be in the inner circle; will be
     ...
     * @param forceDrawDot        The dot in the circle will generally only be shown when the
    ...
    private int reselectSelector(int degrees, boolean isInnerCircle,
        ...
        if (currentShowing == HOUR_INDEX && mIs24HourMode && !isInnerCircle
                && degrees != 0) {
            value += 12;
        }
        return value;
     ...
     * @param forceLegal    Force the selection to be legal, regardless of how far the
     *                      coordinates are from the actual numbers.
     * @param isInnerCircle If the selection may be in the inner circle, pass in a size-1
     ...
     * range. -1 if not.
     */
    private int getDegreesFromCoords(float pointX, float pointY,
                                     boolean forceLegal, final Boolean[] isInnerCircle) {
        int currentItem = getCurrentItemShowing();
        if (currentItem == HOUR_INDEX) {
            return mHourRadialSelectorView.getDegreesFromCoords(pointX, pointY,
                    forceLegal, isInnerCircle);
        } else if (currentItem == MINUTE_INDEX) {
            return mMinuteRadialSelectorView.getDegreesFromCoords(pointX,
    ...
    public int getCurrentItemShowing() {
        if (mCurrentItemShowing != HOUR_INDEX
                && mCurrentItemShowing != MINUTE_INDEX) {
     ...
     *
     * @param animate True to animate the transition, false to show with no
     *                animation.
     */
    public void setCurrentItemShowing(int index, boolean animate) {
        ...
        if (animate && (index != lastIndex)) {
            ...
            if (Build.VERSION.SDK_INT >= 11) {
                ...
                int hourVisibility = (index == HOUR_INDEX) ? View.VISIBLE
                        : View.INVISIBLE;
                int minuteVisibility = (index == MINUTE_INDEX) ? View.VISIBLE
                        : View.INVISIBLE;
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                // it.
                if (!mHideAmPm) {
                    mIsTouchingAmOrPm = mAmPmCirclesView.getIsTouchingAmOrPm(
                            eventX, eventY);
                } else {
                    mIsTouchingAmOrPm = -1;
                }
                if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
                    ...
                    boolean forceLegal = Utils
                            .isTouchExplorationEnabled(mAccessibilityManager);
                    ...
                    if (mDownDegrees != -1) {
                        ...
                        mHandler.postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                mListener.onValueSelected(getCurrentItemShowing(),
                                        value, false);
                            }
                        }, TAP_TIMEOUT);
                    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
                    ...
                    int isTouchingAmOrPm = mAmPmCirclesView.getIsTouchingAmOrPm(
                            eventX, eventY);
                ...
                if (degrees != -1) {
                    ...
                    if (value != mLastValueSelected) {
                        tryVibrate();
                        mLastValueSelected = value;
                        mListener.onValueSelected(getCurrentItemShowing(), value,
                                false);
                    }
                }
                return true;
                ...
                if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
                    ...
                    if (isTouchingAmOrPm == mIsTouchingAmOrPm) {
                        ...
                        if (getIsCurrentlyAmOrPm() != isTouchingAmOrPm) {
                            mListener.onValueSelected(AMPM_INDEX,
                                    mIsTouchingAmOrPm, false);
    ...
    public boolean dispatchPopulateAccessibilityEvent(AccessibilityEvent event) {
        if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            String timeString = DateUtils.formatDateTime(getContext(), millis,
                    flags);
